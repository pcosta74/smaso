...
max.wealth.prod <- function(prod, offset, quant, prices, beta, 
  ...
  if (week>=101) {
    ...
    for(iteration in 1:1000) {
      quantile  <- quantile(c(min, max), probs=c(0,0.25,0.5,0.75,1))
      ...
      cat("Quantities: ",quantile,"\n")
      cat("Wealths: ",v.wealth,"\n")
      ...
      if (round(min,3) == round(max,3)) {
        ...
        max <- quantile[2]
      } else if (which.max(v.wealth) == 2) {
        min <- quantile[1]
        max <- quantile[3]
      } else if (which.max(v.wealth) == 3) {
        min <- quantile[2]
        max <- quantile[3]
      } else if (which.max(v.wealth) == 4) {
        min <- quantile[3]
        max <- quantile[5]
      } else if (which.max(v.wealth) == 5) {
        min <- quantile[4]
